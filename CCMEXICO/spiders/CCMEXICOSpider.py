...
class CCMEXICOSpiderSpider(scrapy.Spider):
    ...
    def parse_item(self, response):
        ...
        item.add_xpath(
            "title",
            '//div[contains(@class, "single-blog")]//h3[contains(@class, "title")]/text()',
        )
        item.add_xpath(
            "author",
            '(//div[contains(@class, "single-blog")]//p[contains(@class, "text-align-right")]/text())[2]',
        )
        item.add_xpath(
            "date",
            '(//div[contains(@class, "single-blog")]//p[contains(@class, "text-align-right")]/text())[1]',
        )
        item.add_xpath(
            "text",
            '//div[contains(@class, "single-blog")]//p[not(@class)]//text()',
        )
